...
* v3.5.3: Updates/fixes include:
    ...
    - Enhanced tests script (/configure/scripts/blacklist.t)
    - Fixed issue with install that prevented admin user configuration
    - Installer checks and repairs active configuration directory group ownership if necessary
...
| 5 | PURGE   |Cleans up stale config sessions|
| 6 | QUIT    |Exit the installer|
...
CLI commands to configure the ADBlock Blacklist:
    ...
    # set service dns forwarding blacklist hosts source hpHosts description 'hpHosts optimized in hosts format'
    # set service dns forwarding blacklist hosts source hpHosts prefix '127.0.0.1 '
    # set service dns forwarding blacklist hosts source hpHosts url 'http://hosts-file.net/download/HOSTS-Optimized.txt'
